return {
  ...
  config = function()
    ...
    local function groq_universal()
      invoke_llm_and_stream_into_editor({
        url = "https://api.groq.com/openai/v1/chat/completions",
        model = "llama3-70b-8192",
        api_key_name = "GROQ_API_KEY",
        system_prompt = universal,
        -- system_prompt = prompts.miyuri,
        replace = true,
      }, make_openai_spec_curl_args, handle_openai_spec_data)
    end
    ...
    --     system_prompt = helpful_prompt,
    --     replace = false,
    --   }, make_anthropic_spec_curl_args, handle_anthropic_spec_data)
    -- end
    --
    ...
    -- vim.keymap.set({ "n", "v" }, "<leader>a", groq_universal, { desc = "llm groq_helper" })
